...
if (existsSync(resultsPath)) {
  try {
    ...
    failures = [{ project: 'Unknown', test: 'Unknown', error: 'Failed to parse test results' }]
  }
}
...
await transporter.sendMail({
  ...
  to: ALERT_EMAIL,
  subject: `[ALERT] ${failures.length} production test(s) failed`,
  html,
})

console.log(`Alert sent to ${ALERT_EMAIL} with ${failures.length} failure(s)`)
