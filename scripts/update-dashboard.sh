...
for REPO in $REPOS; do
  ...
  if [ "$COMMIT_COUNT" != "$CURRENT_COMMITS" ] || true; then
    ...
    UPDATED_DATA=$(echo "$UPDATED_DATA" | jq --arg r "$REPO" \
      ...
      '(.products + .tools) |= map(if .repo == $r then .commits = $c | .lastActive = $d else . end)')
    CHANGES=$((CHANGES + 1))
  fi
done
...
if [ "$CHANGES" -gt 0 ]; then
  echo ""
  echo "Updating data.json with ${CHANGES} changes..."
