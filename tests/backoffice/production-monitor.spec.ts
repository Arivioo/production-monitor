...
test.describe('BackOffice — Production Monitor', () => {
  ...
  test('Health Monitor: project cards load with status indicators', async ({ page }) => {
    ...
    if (result === 'data') {
      ...
      const projectCards = page.locator('.grid .rounded-lg').filter({ hasText: /predivo|backoffice|shipsolo|ytmigration|benchmarksignal|signalscore|replyflow|scoutcopilot|arivioo/i })
